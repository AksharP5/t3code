...
import { vcsRefsRevisionAtom } from "./vcsRefInvalidation.ts";
...
describe("source control environment atoms", () => {
  it.effect("invalidates cached refs after publishing a repository", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const client = {
          [WS_METHODS.sourceControlPublishRepository]: () => Effect.succeed(PUBLISH_RESULT),
        ...
        const revision = vcsRefsRevisionAtom({ environmentId: TARGET.environmentId });

        expect(registry.get(revision)).toBe(0);
        ...
        expect(registry.get(revision)).toBe(1);
